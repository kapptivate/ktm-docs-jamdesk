...
- **Grouping**: organize results by product, test, or monitoring resource.
- **Columns**: adjust the number of columns to fit your screen.
...
- **Show**: display each item's status or its availability percentage.

![The Display menu open over the card layout, with Cards selected and options for grouping, columns, execution mode, bar chart style, and empty periods](/images/realtime-status-display.webp)
...
![The list layout showing three condensed columns of monitors, each row with a status icon and colored dots for recent executions](/images/realtime-status-list.webp)

## Time range and execution mode

Whether you're checking the last few hours after a deploy or reviewing a quarter of history, the analyzed period is yours to set. The time range picker lets you select preset ranges (2 hours, 4 hours, 12 hours, 24 hours, 48 hours, 7 days, 30 days, 90 days, or 12 months) or define a custom range with a precise start and end. This controls the data shown in cards and uptime charts.

![The time range dropdown open, listing presets from Last 2h to Last 12 months](/images/realtime-status-time-presets.webp)
...
In time-based mode you can also zoom directly on a chart: click and drag across the bars to narrow the view to that period. The time range picker switches to "Custom", and you can keep zooming until you isolate the moment you're after.
...
![A custom time range applied, with a tooltip over one bar showing the period and its success, failed, and unexecuted counts](/images/realtime-status-custom-range.webp)
...
- **A value**: picked from a searchable dropdown of your products, tests, or devices.
...
![The Edit filters dialog with an If rule on product, three chained "and" rules excluding tests, and two "or" groups matching products exactly](/images/realtime-status-filters.webp)
