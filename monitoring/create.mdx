...
Create a monitor from the Tests workspace or directly from the Monitors list. A monitor packages a test with a schedule, device assignment, and alert rules so it runs continuously in production.

## Create a new monitor

<Steps>
<Step>
Open **Monitors** and click the **Create** button, or open a test, select it in the context menu, and choose **Create Monitors**.
</Step>
...
<Step>
**Device**
- Select the device(s) the test will run on: SIM, Smartphone, or Ethernet agent.
- Only appears if the test requires devices.
</Step>

<Step>
**Scheduling**
- Choose simple (every X seconds/minutes/hours) or advanced scheduling (specific days, times, timezones).
- Default is every 5 minutes.
</Step>

<Step>
**Options** (optional)
- Enable auto-retry (1–3 retries) if the test fails.
...
<Columns cols={2}>
  <Card title="Scheduling a monitor" icon="calendar-days" href="/monitoring/scheduling">
    Learn simple and advanced scheduling options: frequencies, days, specific times, and timezones.
  </Card>
  <Card title="Tests overview" icon="zap" href="/tests/overview">
    Understand how tests work before promoting them to monitors.
  </Card>
</Columns>
