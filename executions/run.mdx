...
title: Run a test
description: >-
  Queue, review, and launch tests on demand from the tests list or builder, then
  track results in the executions list.
...
Click **Open** on the floating bar to slide out the **Run queue** panel. Each queued test shows its step count and variables. Expand a test to adjust its parameters for this run only, without touching the test itself: the device it runs on, its [variables](/tests/variables#override-values-for-one-run) (the test's own as well as the product's, and an override takes references and randoms, not just plain text), and the channel-specific settings. A web test, for example, exposes its starting URL and its [browser preset](/tests/browser-presets).
