...
After creation, a modal confirms *"Your new token has been generated"* and displays the token. Copy it immediately with the copy button. It's only visible this one time. If you lose it, you'll need to [rotate the token](/administration/api-keys/manage#rotate-a-token) to generate a new one.
...
<Warning>
The token is shown only once at creation time. If you close the modal without copying it, you must rotate the token to get a new one.
</Warning>
