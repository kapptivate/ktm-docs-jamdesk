...
<Note>
  This procedure covers the latest generation of Kapptivate robots. Contact support if yours is older.
</Note>
...
A rack earns its place here. You reach the back of the robot without unplugging anything, and nothing starves the fans. A shelf works too, if that's what the site has. Slide the robot into a free rack slot. Tighten two screws on each side. The front panel takes air in, the back panel pushes it out, so keep both clear.
...
The cabling holds when the robot turns **Online** in the platform, and power alone won't get it there. [Configure its network](/hardware/configure-network) to give it a static IP. A smartphone robot then needs each of its phones prepared, on [Android](/hardware/prepare-android) or [iPhone](/hardware/prepare-iphone).
...
<Columns cols={2}>
  <Card title="Configure the robot network" icon="gear" href="/hardware/configure-network">
    ...
    No robot reaches **Online** without its static IP, whatever its type
