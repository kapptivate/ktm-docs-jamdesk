...
Check that every flow in the IP requirements document is open, not just the one you suspect. The document gives the command for each flow and how to read its answers. Run them on the robot itself.
