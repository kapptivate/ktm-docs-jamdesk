...
Turn the phone on and go through the Android setup: pick the language and country you want, and handle the accounts however you like. Two screens in that setup need attention.
...
Auto Blocker blocks commands sent over the USB cable, which is how a robot drives a phone. Kapptivate and Auto Blocker can't work together.
...
<Steps>
  ...
  <Step title="Set the developer options">
    ...
    **USB debugging** is the one that makes the phone drivable over the cable at all.
    ...
    <Note>
      On a phone that has been used before, tap **Revoke USB debugging authorizations** in the same list, before you plug it into the robot. Authorizations left over from another robot can get in the way. A phone straight out of the box has none, so skip this.
    </Note>
  </Step>
