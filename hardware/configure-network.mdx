...
A robot with a straight route to the internet has no flows to open at all: it finds Kapptivate on its first boot. Expect nothing on the platform yet, though. It appears there once support has [deployed it](#ask-support-to-deploy-the-robot).

Anywhere else, the robot sits behind a filter and flows have to be opened toward Kapptivate. Support keeps that list current, so ask support@kapptivate.com for the IP requirements document, then hand it to your IT department.

Ask for that list once the robot is addressed, not before: every flow is opened from a fixed IP address, so [set the static IP](#set-the-static-ip) comes first.
...
Go with a static IP. [Set the static IP](#set-the-static-ip) below does exactly that, and it pays off twice: your flows open from an address that never moves, and support always knows where to reach the robot.

DHCP works too, and a simple LAN is where it earns its keep: the robot takes a lease at boot, with nothing for you to configure. The cable still has to sit in Ethernet port 1, 2, or 3, since port 4 does not work.

You skip the addressing, not the rest of the page. Checking that the robot reaches Kapptivate and reading its serial number both happen on the robot, so plug in the screen, keyboard, and mouse and log in with the robot's credentials, as steps 2 and 3 describe. Then go to [check that the robot reaches Kapptivate](#check-that-the-robot-reaches-kapptivate).
...
Open a terminal on the Linux desktop. [Network troubleshooting](/hardware/network-troubleshooting) carries the commands to run and the output to expect.
...
<Note>
  Changing the address of a robot that is already deployed? Nothing to ask support. It comes back **Online** on its own after the reboot. If it stays **Offline**, work through [network troubleshooting](/hardware/network-troubleshooting).
</Note>
