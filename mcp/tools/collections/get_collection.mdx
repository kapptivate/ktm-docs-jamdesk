...
```text
Fetch a single collection's metadata plus its breadcrumb path (ancestor chain from the root down to this collection). Useful before mutations so agents can confirm scope. Root UID from list_products detail=true (tests_root_collection_uid).
```
