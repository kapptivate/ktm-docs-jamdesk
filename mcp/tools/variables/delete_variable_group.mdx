...
```text
Safe by default: refuses if the group still contains variables, listing them. acknowledge=true is required. force=true bypasses the local non-empty check; the backend's own refusal still applies.
```
