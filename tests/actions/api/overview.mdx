...
API testing in Kapptivate covers REST and HTTP services: methods, headers, JSON bodies, response assertions, variable extraction, and detailed per-request timing. API actions live as steps inside web or mobile tests, added from the [test builder](/tests/builder) like any other step; the action and metrics are identical in both, only the surrounding test type differs.
