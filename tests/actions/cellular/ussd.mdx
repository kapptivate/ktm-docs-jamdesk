...
|-----------|------------|----------|-------------|
| Access technology | `command.access_technology` | No | Network technology to use (2G, 3G, 4G...). Default: automatic. |
| Command | `command.input` | Yes | The USSD code to send, for example `#100#`. Supports variables. |
| Expected Message | `expected_result.message` | No | The response you expect. The result records how closely the actual response matches. |
| Enable strict text comparison | `command.enable_strict_mode` | No | Exact comparison instead of fuzzy matching. Default: off. |
| Execute multiple times | `command.send_multiple_times`, `command.repetition_number` | No | Repeat the command 1 to 30 times. |
