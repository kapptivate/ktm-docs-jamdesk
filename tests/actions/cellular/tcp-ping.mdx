...
|-----------|------------|----------|-------------|
| Url | `command.url` | Yes | Host (and port) to reach. |
| Ping count | `command.count` | No | Number of connection attempts. Default: 10. |
| TCP Ping should fail | `expect_failure` | No | Invert the verdict. |
