...
|-----------|------------|----------|-------------|
| Use default server | `command.use_default_server` | No | Let Ookla pick the closest server. Default: on. |
| Server hostname | `command.server_hostname` | When default server is off | A specific Ookla server to test against. |
