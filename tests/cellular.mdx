...
**Input variables** are parameters you pass to your test. You can fill them with product-level defaults or override them at suite level.

**Owner variables** specify which SIM runs which action. You can override owner assignments at suite level, so the same test can run on different SIMs in different campaigns.
...
A **suite** (or campaign) groups multiple tests and lets you run them together with shared configuration.

**Suite features:**

- **Test list** — Order the tests to run
- **Variable overrides** — Set input variable values for all tests in the suite (override test defaults)
- **Owner overrides** — Assign which SIM runs each action across all tests in the suite (override test owner assignments)
- **Execution options** — Control timeouts, SMS observation delay, and optional reception flags for each test

**Example:** You have three tests (balance check, top-up, history) and two SIMs (prod, staging). Create a suite, add all three tests, override the owner to `staging`, then run once. Later, override the owner to `prod` and run again on production SIMs.

## Running tests

### Configure the run

1. Select your test or suite
2. Click **Run**
...
<Columns cols={2}>
  <Card title="Channels & test types" icon="shuffle" href="/concepts/channels">
    Learn about cellular and other test channels
  </Card>
  <Card title="Agents" icon="robot" href="/concepts/agents">
    Understand cellular agents and how they run on-premise
  </Card>
</Columns>
