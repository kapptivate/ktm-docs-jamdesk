...
Each cellular test is built from three action types. All run on real SIMs on your cellular agent. This page covers the flow; the [SIM steps reference](/tests/actions/cellular/overview) documents every action's parameters, including the Data and Call actions not shown here.

### [USSD](/tests/actions/cellular/ussd) (send and wait for response)
...
### [SMS_GET](/tests/actions/cellular/wait-sms) (wait for incoming SMS)
...
### [USSD_PUSH](/tests/actions/cellular/ussd-push) (wait for unsolicited USSD from network)
...
A **campaign** (also called a [suite](/tests/suites)) groups multiple tests and lets you run them together with shared configuration.
...
<Columns cols={2}>
  <Card title="SIM steps reference" icon="sim-card" href="/tests/actions/cellular/overview">
