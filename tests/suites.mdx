...
Click **Run** from the campaigns table or the editor. The **Run confirmation** page opens for a last review: name the execution (it defaults to the campaign name), adjust each test's owner device and input variables if needed, then click **Confirm run** (top right).

Queued runs appear in the **Run queue** sidebar, where **Clear queue** empties all pending runs after a confirmation. The campaign lands in the [executions list](/executions/overview) and opens as a [multi-test execution](/executions/results#multi-test-executions).
