...
- **Run**: queue the campaign for execution.
...
- **Global variables**: override values for this test only, in this campaign only.
...
Click **Run** from the campaigns table or the editor. The **Run queue** slides out as a right sidebar showing the queued campaign cards; **Clear queue** empties all pending runs after a confirmation. While tests run you can follow their status, and past runs are available from the campaign's execution history.
