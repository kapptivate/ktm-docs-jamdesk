...
It is the same modal as the one in the [mobile app recorder](/tests/mobile-recorder#find-an-element), so the mechanics are described once there: how the selectors are ranked, what a shared selector means, and how to read a point in **Coords** mode. In a live session there is no test being edited, so the selector is yours to copy into the step you are building elsewhere.
