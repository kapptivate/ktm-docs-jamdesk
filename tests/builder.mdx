...
Click **Add step...** at the bottom of a step group to insert a step without recording. Pick the action from the catalog, then configure its target and value. See the [Actions Library](/tests/actions/overview) for every available step.
...
**Link to test** ties the step to the test-level AI setting instead of a per-step override. The test-level toggle and strategy live in **Test settings > AI**.
...
Click the `{}` button in any text field (selector, value, URL) to insert a variable. You can mix literal text and variables in a single field, for example `https://example.com/{userId}`.

Global variables and variable groups are managed per product in [Configurations & variables](/tests/configurations).
