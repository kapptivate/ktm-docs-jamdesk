...
Browse normally: clicks, inputs, and navigations are captured as steps in real time. When you stop, the recorded steps appear in your step canvas, ready to edit. See the [web recorder](/tests/web-recorder) for the full flow, including the side panel and recording verifications.
...
Smartphone tests record through a cloud-based recorder instead of a browser extension: you drive the real device from your browser and your taps, inputs, and scrolls become steps. See the [mobile app recorder](/tests/mobile-recorder) for the side-by-side recording flow.
...
- **Browser preset**: the browser configuration for web tests (e.g., "Chrome (Desktop, Chrome 143.0)"). A new test opens on your [default preset](/tests/browser-presets#set-a-default-preset). Click it to pick from your saved [browser presets](/tests/browser-presets).
...
- **Advanced**: the test's default [execution mode](/executions/execution-modes) (how much data each run collects), accepting non-secure certificates, custom capabilities, and **Reset all selectors**

**Reset all selectors** clears every selector the AI has found and cached in this test, so the next run looks for the elements again. Reach for it after a redesign, when cached selectors point at elements that no longer exist. Steps that come from a [reusable component](/tests/reusable-components) keep their own selectors.
