...
/** Resolve a runnable ktm binary from $KTM_BIN. In CI it's installed via the official
 *  install.sh (see .github/workflows/sync-docs.yml); locally, point KTM_BIN at a build. */
function ensureBinary({ bin }) {
  if (bin) return bin;
  throw new Error('CLI: set KTM_BIN to a ktm binary path (install via the ktm install.sh, then point KTM_BIN at it)');
}
...
export async function introspectCli({ mode = 'binary', bin, file }) {
  if (mode === 'file') {
